...
def test_d8_template_topology_and_protected_defaults():
    ...
    # D8 is an identity-topology fence, not a dependency-artifact hash scanner. Keep
    # the repo-wide non-test production scan, excluding only generated dependency lock
    # material whose digest fragments can legitimately look like uid/gid literals.
    diff = subprocess.run(
        [
            "git", "diff", "--unified=0", base, "HEAD", "--",
            ":!tests/", ":(exclude)requirements/*.lock",
        ],
