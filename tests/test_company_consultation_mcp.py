...
_FROZEN_COMPANY_CONSULTATION_TOOL_SCHEMA_DIGEST = (
    "f9463e714240c5ac347bb029788b88d5556ad9d77c69eabcec1b7038e210a722"
)


def test_public_tool_schema_digest_and_peers_projection_are_frozen() -> None:
    assert COMPANY_CONSULTATION_TOOL_SCHEMA_DIGEST == _FROZEN_COMPANY_CONSULTATION_TOOL_SCHEMA_DIGEST
...
def _grok_peer(peer_ref: str = "peer-9bdf4a6f9a664bbcf1a93d67a41ba51d") -> ConsultationPeer:
...
def test_codex_peer_derives_v1_and_grok_bot_peer_derives_v2() -> None:
    assert _peer().consultation_schema == "mastermind.agent_dialogue_consultation.v1"
    assert _grok_peer().consultation_schema == "mastermind.agent_dialogue_consultation.v2"


def test_company_consult_dispatch_request_carries_the_derived_consultation_schema() -> None:
    grok_peer = _grok_peer()
    gateway, sink = _gateway([grok_peer])

    response = _run(
        gateway.call(
            "company.consult",
            {
                "to": grok_peer.peer_ref,
                "question": "What fields are forbidden?",
                "evidence_refs": [],
                "artifact_revisions": [_artifact()],
            },
    ...
    assert response["ok"] is True
    assert sink.calls[0][1]["consultation_schema"] == "mastermind.agent_dialogue_consultation.v2"


def test_caller_cannot_provide_or_override_consultation_schema() -> None:
    gateway, sink = _gateway()
    ...
    response = _run(
        gateway.call(
            "company.consult",
            {
                ...
                "evidence_refs": [],
                "artifact_revisions": [_artifact()],
                "consultation_schema": "mastermind.agent_dialogue_consultation.v2",
            },
        )
    )
