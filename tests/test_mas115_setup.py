...
def test_build_enrollment_document_is_exact_three_distinct_seats_and_preserves_unrelated():
    ...
    doc = setup.build_enrollment_document(
        {"schema": sb.SCHEMA, "bindings": [unrelated, old_chatgpt]},
    ...
    assert {row["work_ref"] for row in chatgpt_rows} == {setup.WORK_REF}
    assert {row["role"] for row in chatgpt_rows} == {"ceo"}
    assert all(row["observed_at"] == "2026-08-23T12:00:00Z" for row in chatgpt_rows)
