...
WAKE_EVENTS = ROOT / "control_plane/wake_events.py"
...
def test_exception_owner_map_and_no_rebuild_are_closed() -> None:
    ...
    assert contract["architecture_exception"] == {
        ...
        "concrete_blocker": (
            ...
            "compare-and-swap semantic directive binds one exact terminal return to one "
...
def test_acf1_reuses_the_exact_existing_action_target_owner() -> None:
    ...
    assert acf1["actor_authority_owner"] == {
        ...
        "transaction_law": [
            ...
            "bind alias, binding id, generation and reasoning surface from the same resolution",
            (
...
def test_direct_chairman_runtime_actor_is_deliberately_absent_from_v1() -> None:
    ...
    assert supersession["direct_machine_authenticated_chairman_actor_in_v1"] is False
    assert supersession[
        "fresh_chairman_intervention_is_submitted_by_then_current_action_target_as_revision_n_plus_1"
    ] is True
    assert supersession["projection_reason"] == "DIRECTIVE_SUPERSEDED"
    serialized = json.dumps(contract, sort_keys=True)
    assert "CHAIRMAN_SUPERSEDED" not in serialized
    assert "CHAIRMAN_AUTHORITY_INVALID" not in serialized


def test_decision_body_union_is_exact_bounded_and_uses_existing_canonicalizer() -> None:
    acf1 = _contract(DESIGN)["acf1_semantic_directive_convergence"]
    ...
    assert body["canonicalizer"] == "control_plane.wake_events.canonical_json_bytes"
    ...
    assert repair["repair_ref"]["owner"] == "common.commission_ref.normalize_commission_ref"
    ...
    assert "def canonical_json_bytes(" in _read(WAKE_EVENTS)
    assert "def normalize_commission_ref(" in _read(COMMISSION_REF)


def test_all_body_variants_have_stable_canonical_payload_digests() -> None:
    variants = {
        "CONTINUE": {"schema": BODY_SCHEMA, "decision": "CONTINUE"},
        "REPAIR": {
            "schema": BODY_SCHEMA,
            "decision": "REPAIR",
            "repair_ref": {
    ...
    assert _payload_digest("CONTINUE", reordered) == digests["CONTINUE"]
    changed = dict(variants["STOP"])
...
def test_commit_supersession_consumer_and_results_are_closed() -> None:
    ...
    assert supersession["consumed_applied_or_effect_unknown_requires_reconciliation"] is True
    ...
    assert consumer[
        "consumption_and_downstream_mutation_share_one_existing_transaction"
    ] is True
...
def test_follow_on_layers_stay_evidence_gated_and_acf1_stays_unassigned() -> None:
    ...
    assert contract["current_state"]["worker_assigned"] is False
    assert "No implementation worker or successor wave may start" in _read(DESIGN)
    assert "WAITING_RUNTIME_PATH_RELEASE" in _read(PLAN)
