"""The China desk's MCP surface — the single-name A-share book the Brain manages itself.

Sibling of ``brain/autonomous_mcp.py`` but pointed at the macro China desks and the mainland
single-company A-share universe. The Brain researches via the China read tools below + the web,
...
@tool("submit_book",
      ...
      "liquid, positively verified single-company mainland A-shares ONLY (ticker like 600519.SS / "
      "300750.SZ). ETFs, index funds, pooled products, warrants, Hong Kong (*.HK), and US-listed "
...
@tool("get_context_catalog",
      ...
      "planes. Freshness and authority are explicit; use China intake/regime for market selection.", {})
...
@tool("get_surface_packet",
      ...
      "context only: never transplant a US-market read into the China regime or grant it trade authority.",
...
@tool("get_technical_lab",
      ...
      "one China ticker. Missing fields stay explicit and technicals never override the China intake.",
...
@tool("get_neural_web_packet",
      "Bounded Neural Web context for this China book and up to six requested/held names. Context "
      "and provenance only; it cannot originate, size, block, or exit a trade.",
...
@tool("request_context_upgrade",
      "Queue a bounded request for missing China decision context. Review only: it does not change "
      "code, tools, sizing, or execution authority.",
      {"type": "object", "properties": {
