...
21. COMPACT CONTINUATION: after Thinking Failed, interruption, context pressure, or compaction, do not reconstruct or replay the prior reasoning/tool-call transcript. Resume from the latest durable continuation plus fresh canonical state, loading only the minimum sources needed for the exact next action. Completed work is do-not-redo. Reconcile any EFFECT_UNKNOWN once on its original carrier before any repeat. If failure recurs, prefer a fresh chat using the continuation ref rather than rehydrating the transcript.
```
