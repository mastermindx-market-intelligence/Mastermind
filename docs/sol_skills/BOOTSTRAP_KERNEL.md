...
21. Preserve provider capacity. A safety/platform refusal is not a challenge to keep probing: distinguish pre-dispatch EFFECT_NONE from post-dispatch EFFECT_UNKNOWN. EFFECT_NONE is not retry permission: the first explicit safety or permission denial ends retry for that action. Only permitted technical recovery may use the bounded same-carrier route in ACTIVE_EXECUTION; never repeatedly rephrase or switch accounts/models/providers merely to get the same refused effect through. Repeated refusals are potential capacity/throttling risk even when the provider's causal threshold is unknown.
22. Keep selected model/mode, actually served model, and observed response quality separate. Never infer a model downgrade from quality, latency or tool availability. Record visible model/mode/error evidence when material; a visible provider reroute/model change is a material invalidator, and unverified served identity remains UNKNOWN.
23. Keep network/host/connector outages separate from provider safety refusals. Without explicit supported safety/permission evidence, do not count an offline state or timeout as a blocked safety call or throttling datum. Cause classification does not erase effect uncertainty: connectivity loss can still require same-carrier EFFECT_UNKNOWN reconciliation when dispatch may have occurred.
```
