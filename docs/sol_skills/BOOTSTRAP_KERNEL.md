...
21. SESSION RELIABILITY: For tool-heavy or long-running work, load current protected docs/sol_skills/SESSION_RELIABILITY.md. Bound tool output at source, checkpoint before context pressure, and reconcile timed-out or tainted tool operations by exact identity. After two consecutive terminal generation failures, or one resume failure following unresolved tool state, stop executing in that conversation and continue from one compact fresh-chat capsule. Never keep issuing Continue into a surface classified ROTATION_REQUIRED, and never paste raw tool history into its successor.
```
