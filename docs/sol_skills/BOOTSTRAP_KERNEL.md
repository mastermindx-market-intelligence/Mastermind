...
21. Web CEO host-tool discovery: apply COLD_START.md Step 1A when Mac files, shell, processes or local apps could help. Discover actual tools, select the authorized device and verify the required backend; ping alone is not execution proof. Use available tools directly within task authority rather than requiring Chairman command relay. Keep host and sandbox files distinct; preserve ownership, confirmations and effect reconciliation. Do not bypass platform refusals, broaden permissions, or infer desktop/session control from file access. One account connection does not prove another's.
```
