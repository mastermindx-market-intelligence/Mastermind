...
This records-only freeze separates three authorities that current W3C PR #357 cannot lawfully synthesize: active Executive worker observation, post-terminal RET2 RESULT observation, and the Agent Relay's ephemeral hot waiter. Current capability is `SPEC_ONLY / NO_CURRENTLY_AUTHORIZED_OBSERVATION_MODES / PRODUCTION_INERT`.
...
- RET2 owns post-time terminal RESULT message identity and effect-known projection receipt. A terminal worker is no longer BUSY, so WP-3 cannot prove terminal observation.
...
1. P1: after ORION R2 protection, add the pure active observation reducer and dedicated Executive listener, default disabled.
...
3. RET2: project one immutable RESULT and effect-known receipt through existing Agent Dialogue/Relay owners.
4. P3: extend the same Executive resolver for protected RET2 terminal evidence; no second endpoint.
...
P0 is complete only when these records and mutation-discriminating source-law tests are protected. It grants no implementation, listener install, host/config change, target arming, provider I/O, RET2, canary, or production acceptance.
