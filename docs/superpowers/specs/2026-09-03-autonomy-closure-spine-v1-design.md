...
Runtime must call the existing `control_plane.sol_action_target.require_sol_action_authority(...)` using the exact root, current `SessionTargetRegistry`, complete current `RuntimeBindingSnapshot`, and actor binding. It binds the resulting alias, binding id, generation, reasoning surface, and `SolActionTargetResolution.evidence_digest`, then re-resolves immediately before a **new Event append** because the resolution is evidence, not a reusable capability.
...
Exact replay with the same complete semantics is idempotent. Changed semantics under the same command identity are `DIRECTIVE_REPLAY_CONFLICT`.

## Replay-first readback law

Command lookup occurs before actor revalidation.

When an exact existing Event is found, Runtime validates the immutable Event bytes, command binding, terminal-return identity, payload digest, and the authority receipt recorded when the effect was committed, then returns `IDEMPOTENT_REPLAY` with no new mutation. It does **not** require the caller to remain the current action target merely to read back an already-committed effect.

This distinction is essential for response-loss recovery: target rotation after commit cannot make the historical Event unknowable or convert a known effect into permanent uncertainty. Current target/actor revalidation is required only before a new append or a superseding revision. A changed command payload still conflicts.

Replay never grants authority for another action. It only reconstructs a prior immutable effect.
...
For a new directive, inside one existing `BEGIN IMMEDIATE` transaction, Runtime revalidates the exact terminal Event, current Attempt/Worker lineage, current action target, actor receipt, predecessor revision, and absence of a sibling effective directive; then it appends one command-unique Event.
...
This source-child-only STOP boundary prevents a small child-return directive from ambiguously cancelling the whole root. Root completion, cancellation, or retirement continues through its existing owner.
