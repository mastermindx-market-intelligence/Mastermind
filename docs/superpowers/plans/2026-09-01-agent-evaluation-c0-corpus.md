...
seal_status: PROVENANCE_PLACEHOLDER_BODY_UNRECOVERABLE | SEALED_BODY_DELIVERABLE
...
**`seal_status` (principal ruling, MAJOR-3 repair, added 2026-09-01): honest provenance, never an ad-hoc private store.** The first adversarial review of this PR flagged that all three seals' `sealed_body_digest` values were computed over a bundle that was then permanently discarded — the digest is real (it genuinely was computed over a real in-memory bundle at authoring time), but that exact bundle can never be re-derived, recomputed, or delivered by anyone, ever. The principal ruling was explicit: do **not** mint an ad-hoc private store to work around this (that would be exactly the kind of undisclosed, unreviewed private-evidence side-channel this fabric's design forbids). Instead, `seal_status` states the fact plainly:

- **`PROVENANCE_PLACEHOLDER_BODY_UNRECOVERABLE`** — the value on all three seals this PR ships. The digest is a real cryptographic commitment to a real bundle that once existed in this build's working memory; it is a governed *intent-to-seal* record, not delivered evidence, and its body is permanently unrecoverable. **`EVAL-E1` and any later consumer MUST NOT count a `PROVENANCE_PLACEHOLDER_BODY_UNRECOVERABLE` seal as usable holdout evidence** — it proves nothing was leaked (that guarantee still holds: the body never entered the repository), but it cannot be replayed, scored, or delivered as a real evaluation case.
- **`SEALED_BODY_DELIVERABLE`** — reserved for a future seal minted only once a real private evidence root exists to actually receive the body `private_evidence_root_ref` names. No seal in this PR uses this value.

**Delivery contract (explicit, not yet built):** `private_evidence_root_ref` names where a *future, freshly-minted* `SEALED_BODY_DELIVERABLE` seal's body will live once a private evaluation root exists — a later wave's concern, not this one's, and **not** a promise that this PR's own three `PROVENANCE_PLACEHOLDER_BODY_UNRECOVERABLE` bodies will ever appear there (they cannot; they no longer exist anywhere). Until such a root is built and populated, every `private_evidence_root_ref` value in this corpus is **symbolic and intentionally unresolvable**: it documents a destination pattern, not a currently-reachable location. A future wave that builds the private root and mints a real `SEALED_BODY_DELIVERABLE` seal MUST (a) publish that seal's body at exactly the path its own `private_evidence_root_ref` names, (b) recompute `sha256:<digest of that body>` and require exact equality with that seal's `sealed_body_digest` before treating the delivered body as authentic, and (c) never modify a seal's `seal_digest` to accommodate a body that does not hash to the pinned value — a body/seal mismatch is a corruption/tamper signal, handled exactly like any other digest mismatch in this fabric (never silently repaired). This wave builds no private root, no delivery mechanism, and no resolver for `private_evidence_root_ref` — that is out of scope here and is named as a gap below (§9).
...
- **The three holdouts this PR ships are unrecoverable provenance placeholders, not usable holdout evidence (MAJOR-3, principal ruling).** Each of `temporal_cutoff_ordering`, `multi_file_coordinated_fence`, and `named_seat_override_freshness_fence` carries `seal_status: PROVENANCE_PLACEHOLDER_BODY_UNRECOVERABLE` (§6): its sealed body was computed in-memory and permanently discarded before this PR was authored, and can never be re-derived, recomputed, or delivered by any future process — no ad-hoc private store was minted to work around this, on explicit principal instruction. `EVAL-E1` and every later consumer must treat these three seals as governance/leakage-accounting records only (proof that a body was never committed to the public repository) and **must not** count them toward holdout sample size, pair them against a real run, or otherwise treat them as delivered evaluation evidence. A future wave that wants real, usable holdouts for these three task classes must mint fresh seals with `seal_status: SEALED_BODY_DELIVERABLE` once a real private evidence root exists to receive their bodies — that is new work, not a delivery of these three.
...
- **Public-repository exposure and expected decay (NB-9).** Every public case in this corpus is committed, in the clear, to a public GitHub repository the moment this PR merges — including each case's gold `expected_contract`. Any model whose training data later ingests this repository (this one, or a mirror/fork of it) may have seen the exact gold answer for these nine cases; this is not a hypothetical, it is the default outcome of publishing evaluation gold in a public repo. Two consequences follow, both expected and not remediated by this wave: (1) a model's performance on these nine public cases should be read as an **upper bound**, never a clean out-of-sample measurement, once enough time/training cycles have passed for public-repo ingestion to plausibly have occurred; (2) this is exactly why the sealed holdouts (§6) exist as a category — they are the only samples in this task-class design meant to resist this decay — but see the bullet above: the three holdouts *this specific PR* ships cannot serve that role either, because they are unrecoverable placeholders, not delivered evidence. A future wave with real, delivered `SEALED_BODY_DELIVERABLE` holdouts is the actual remedy; refreshing/rotating the public cases periodically is a lighter partial mitigation this record does not commit to on any schedule.
- **TC1 v1 ceiling-effect limitation (NB-7).** All three published `current_source_comprehension` cases are near-pure extraction: the frozen extract, read literally, already states the answer (a precedence rule, a numeric constant, an ownership statement) with no multi-step inference, cross-reference, or synthesis required. This is a deliberate v1 choice (fast to build, unambiguous gold, genuinely replay-safe) but it means TC1 v1 is a weak discriminator between a model that can extract a stated fact and one that can actually reason over a frozen source — most capable models should approach ceiling on these three cases specifically, and a near-ceiling score here should not be read as strong evidence of comprehension beyond literal extraction. A later corpus wave should add harder TC1 cases requiring the model to combine two or more extract passages, apply a stated rule to a novel hypothetical the extract does not directly answer, or notice when a plausible-looking answer is actually absent from the extract.
- **`tests/test_agent_eval_inertness.py`'s changed-path fence is a per-wave surface ratchet, landed for C0 (MAJOR-6 repair).** An earlier draft of this record described the fence as unmodified/R0-only and predicted `test_changed_paths_are_within_the_allowed_r0_surface` would fail on this branch — that was accurate against this PR's *first* commit, but is now stale: principal authorization (2026-09-01, citing operation `mastermind-agent-evaluation-c0-corpus-20260901-fable-001`) extended `ALLOWED_PATHS` with C0's three new exact files and added a `corpus/agent_eval/` entry to a new, separately-constrained `ALLOWED_PATH_PREFIXES` (NB-8: prefix admission is limited to the corpus law's own known filenames — `scenario.json`, `holdout_seal.json`, `corpus_manifest.json`, `fixtures/*.json`, a corpus README — never any file type dropped under the tree). Both changed-path fence tests pass on this branch as committed. The mechanism itself remains a deliberate **per-wave surface ratchet**: every wave's owned paths land in this fence only via its own explicit principal authorization and operation-key citation, never unilaterally by that wave's own worker — a future C-lineage wave widening the corpus further needs its own such authorization, not an assumption that this one already covers it.
