...
**Original source basis:** Mastermind protected `4709b9483182c20153868dac164e1f621aac505c`; Macro main `52bd0cde0669cd8ea396dfbe692399261dd5cfe5`.
**Review-repair inspection pin:** Mastermind protected `7642aea155d2817219135b24246b55c1d7611c66`, Skillpack 1.0.1 / bootstrap 1. The intervening #675 projection-maintenance merge is path-disjoint from this packet. Re-pin at every modifying step; these are inspection identities, not perpetual activation preimages.
**Review basis:** exact-head review `5217359312` and independent review comment `5690413477`, both against `7956f74427d1c2ca65c18d99bd77b60553e159fc`. Repair remains on the same PR #676 / source operation. Scope adds only the static test consumer to the original plan and fixture; no runtime implementation or activation.
...
| Macro #7114 | Open/Draft at `7f68d90d3c5453dc7b2f6adb8fcca379fbd06a0b`; incumbent compiled Claude/Fable profile/guard owner after same-branch CI-wiring repair; full hosted/adoption gates remain. | Re-read current GitHub head before touching it. Safe opt-in conformance is not fleet enforcement; no competing compiler/profile implementation. |
...
| Admission #667 / W1-H3 | Earlier #667 source-release snapshot is historical. Current principal records W1-H3 source child STARTED with repair outstanding; this packet establishes no installed arm receipt. | Reconcile current source and release state. Production admission remains the existing Business App -> CeoIngress -> Executive service path, not the deliberately fixture-only ExecutiveMcpGateway. No armed-config shortcut. |
| Family-B realm/Capacity evolution | Mastermind #662 `30f8a4c1188f19f99622da5503a70b643c1ef167` + Macro #7162 `d5ac0d61f1a95b9d21441a6dc3b1905bde6b7c6d`, both Open/Draft: **PROPOSED / B0 REVIEW-GATED / NOT_ENROLLED**. | Existing-owner evolution toward real realm generation/enrollment and Provider Capacity V2/atomic-claim composition. B0 acceptance and later source/host proof are predecessors; this packet starts neither B1+ nor production. |
...
- **43** user skill-directory entries and **18** user agent Markdown files under the default user's `.claude` home. The independent R676-R1 inventory identifies 33 physical directories and 10 symlinked `bailian-*` directories within those 43; this breakdown is attributed review evidence, not an additional live-profile attestation.
...
**Usage debit is not usage-policy permission.** Headless `claude -p` and equivalent noninteractive SDK execution MUST be classified as unattended under the current Mastermind policy boundary unless an exact current provider-policy receipt explicitly admits that execution mode. Being started by an attended orchestrator does not satisfy `interactive_only=true` when `unattended_background_allowed=false`. Classify Mac UI, desktop-control and SDK routes by actual execution behavior, not their app label. An interactive canary MUST NOT flip unattended or autonomous policy flags. Background production requires an admitted provider-supported unattended realm; the billing notice [S2] is not that admission.

**Current evidence-producer gap.** The v1 realm and Capacity evidence seams are fixture-only/hermetic: `ops/executive_os/provider_realm_facts.py::issue_provider_realm_enrollment_receipt` and `ops/executive_os/capacity_owner_facts.py::export_capacity_fact` refuse direct production issuance, and the current closure seams do not establish a production current-realm owner. A real `provider_realm_enrolled` / `capacity_known` path is not callable merely because these types exist. Wave C MUST NOT promote #581's hermetic v1 facts to production. Consume the accepted current owner-issued realm generation/enrollment and Provider Capacity V2/atomic-claim composition when proven. The paired Family-B candidates in §2 are the current proposed owner evolution, not governing source: Family-B B0 review/acceptance is a predecessor, not permission to START B1+. No substitute receipt store or caller-selected booleans are authorized. Owner-issued current realm generation MUST be exact-realm scoped and checked again at admission; stale, caller-selected or wrong-realm state refuses.

**Shared-only telemetry is insufficient.** The documented Claude Code status-line feed provides shared `rate_limits.five_hour` and `rate_limits.seven_day` values with `used_percentage` / `resets_at`, not a documented Fable-family balance [S9]. Fields may be absent before the first response or disappear after expiry. Shared-window telemetry alone MUST NOT establish Fable capacity when an applicable family limit is absent, stale or unknown. A family observation must come from a qualified, current, version-supported owner path; an experimental control name or cached UI value is only an investigation lead. Preserve native units, denominator, provider-window identity and actual observation time; a config-file mtime never refreshes embedded evidence. Coalesce concurrent refreshes through the existing Provider Control owner with single-flight and backoff. Do not spawn a Claude process per status refresh or extract bearer credentials to bypass an unavailable native read. Q13 covers the shared-only refusal; Q06/Q07/Q09 retain denominator, freshness and correction rules.
...
Model Router first fixes the lawful suitability tier; policy, capability, fresh resource constraints, host fit and continuity remain hard eligibility gates. Ranking MUST preserve strict Pareto dominance before the documented lexicographic ordering for the mission/risk class. Preserve separate native quota debit, explicit cash cost, queue time, expiry, reserve and opportunity-cost dimensions; accepted-result estimates include failed attempts, review and repair. Do not fold unlike units into one universal weighted score. A scalar MAY operate only inside one already-frozen lexicographic stage or as a display aid; it cannot override a prior stage or strict dominance. Static provider preference MUST be a deterministic tie-break only after the documented stages, absent a separately justified typed hard eligibility reason. Incomparable candidates remain incomparable until those declared stages resolve them; no arbitrary provider ranking fills missing evidence. A cheap model that repeatedly requires repair is not necessarily economical.

Host replicas MUST NOT multiply one provider entitlement: every replica consumes the same provider-domain resource identities, while host-local concurrency stays a separate constraint. Shared-resource reservations must be atomic at the accepted claim boundary. Observation reconciliation must not subtract the same usage twice: reconcile reservation identity against a provider observation generation, retain unobserved holds, and quarantine ambiguous consumption. Reset events need provider window identity, not the local weekday. Forecasted rolling-window release is not a confirmed reset. Re-estimate on version/plan changes and keep uncertainty explicit.
...
- `usage_policy_mode` and `provider_policy_receipt_ref`, bound to actual execution mode, exact provider/product/realm, applicable model/harness, policy revision, validity and allowed interactive/unattended behavior; mode compatibility is rechecked at admission, never inferred from billing category or UI presence;
...
1. Reconcile exact plan tier/generation and usage-policy mode. First verify whether the existing observation owner has a qualified native shared-and-family acquisition path. If not, record the §3 production evidence-producer gap and advance only source qualification under the incumbent owner. Capture an authorized, secret-safe observation only after that path and policy are established; unknown remains unavailable.
...
3. Consume fixture cases Q01–Q15 in the existing economics/normalization tests. Include provider-denominator, observation-correction, missing-family, replicated-entitlement and Pareto/lexicographic falsifiers before changing consumer code. The static consumer validates this corpus, not the production algorithm.
...
5. Wave B live before/after proof depends on Wave C admission. After that gate is accepted, prove a legitimate quota-driven choice, actual before/after usage and reservation reconciliation on the same governed task. Source qualification can precede C; no live task is implied by B alone and no extra inference is created merely to burn expiring quota.
...
Owner: existing adapters/role compiler/Capacity/RuntimeBinding owners. Host telemetry is consumed only from qualified existing host/Workbench owners, never inferred from provider quota.

Host-dimension custody: memory/CPU pressure, free disk and network health require an existing enrolled host/Workbench observation source with age and identity; this packet has not proven those producers and marks them **NOT_BUILT / owner-integration gap** for routing until verified. Workspace ownership/free capacity comes from existing `mmx-workspace`/workspace leases; runtime/tool/OS compatibility from the qualified launch profile and enrolled realm; permission from existing grants; desktop exclusivity from the existing lease owner. Presence of those owners does not prove all their proposed exports exist. Missing mandatory observations refuse that placement; fixed placement may continue only under an independently accepted known-host contract, never fabricated balancing. No new host telemetry collector is authorized by this plan.
...
Waves A and B may advance source-only on disjoint, unowned paths while admission is blocked. C needs the real admission/realm/tool/capacity/mode gates, not hermetic fact minting. B's live measurement joins C only after admission. D depends on C and role/continuity proof. E expands qualified coverage, not a replacement architecture. #675/VTP-M1 is protected; do not restart its terminal child. Preserve other active children and their source custody. #7116's known publication refusal remains an explicit boundary, not permission to republish its repair from this packet.
...
- S9 — Anthropic, Claude Code status line: https://code.claude.com/docs/en/statusline . Rechecked 2026-09-15 America/New_York / 2026-09-16 UTC. The documented shared five-hour/seven-day feed is not a Fable-family observation; missing/expired fields do not grant capacity.
