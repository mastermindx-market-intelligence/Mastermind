...
- `evidence/EVIDENCE_DISPOSITION.md` — what happened to every capture, and why.
- `evidence/removed_assets.jsonl` — SHA-256 identity of each removed capture.
- `evidence/screens_sanitized/` — the retained tight crops, named by observation ID.
...
- Focused screenshots taken: 30. Retained after the rights-safety repair: 5 tight
  crops. Removed: 25 (23 restricted, 2 duplicates). See
  `evidence/EVIDENCE_DISPOSITION.md`.
...
## Evidence-rights note

The reconnaissance was accepted; its packaging was repaired afterwards. Fiscal.ai
is a data product, so on most routes the screen *is* the licensed corpus —
standardized statements, consensus estimates, compiled ownership tables,
third-party research, transcripts, investor decks and fund letters. Canonical Git
should not become a durable copy of that, so those captures were removed rather
than blurred, and their SHA-256 identities were kept so the captures stay
provable. What survives is the structured record: route, user job, interaction
pattern, context transfer, gating/null/loading/failure semantics and product
insight. Where a record reads as though a reader could look at the frame, the
frame is gone on purpose and the record says so.

Retained crops are limited to states that hold no data by construction — empty
states, loading states, an explicit no-match dialog, one product-failure row of
public quote fields, and this run's own user-generated metric.
