...
| **RTO** (declared start → verified read-only Executive availability) | **≤ 4 h** with the runbook, measured by drill | single-DB restore + verification chain is minutes; the budget is dominated by host provisioning |
...
Targets are re-measured by every DR-D1 drill; drift between target and measured is a failure state, not a footnote.
...
- **The key never enters a model context, git, Slack, Linear, a manifest, or a log — and no fleet session generates or copies key material at all** (session-side key handling is blocked by the harness, and rightly: custody that survives host loss requires the Chairman anyway). Custody design:
  ...
  2. **Standing key (production):** provisioned in ONE Chairman ceremony command (runbook §ceremony): generate → password-manager/offline copy → GitHub Actions secret `EXECUTIVE_DR_KEY_V1` → install 0400 key file for `_mastermind_exec` on the host. Until that ceremony, the DR-PROMOTE ruler's key-custody item is honestly **PARTIAL**.
...
- **Drill lane (no credential provisioning at all):** the DR-D1 workflow ships exports as release assets on the Mastermind repo itself under the reserved tag namespace `dr-export/*`, authenticated by the workflow's **ephemeral, repo-scoped `GITHUB_TOKEN`** (`permissions: contents: write`) — least-privilege by construction, zero long-lived credential, zero token copying. Census confirms the repo's release namespace is otherwise unused.
- **Production lane:** dedicated private repo `mastermindx-market-intelligence/executive-dr-vault` (created 2026-09-01) as the sole DR object store — one release per export, asset = encrypted artifact, body = envelope JSON — written by a **fine-grained PAT scoped to that one repo**, provisioned by the Chairman at the arming ceremony (runbook item; no broad token is ever stored for this).
- Rationale: the only failure-domain-independent store writable today without credential enrollment; GitHub already holds the company's implementation bytes, so no *new* trust anchor is introduced; assets are content-checksummed after upload (re-download digest compare).
- Create-only is enforced at the application layer: the uploader refuses when a release tag or asset name already exists (conflict = typed failure), and neither lane carries delete tooling; deletion is a privileged maintenance action with receipts.
...
3. Measures and publishes RPO (age of newest valid export), RTO (retrieve→verified), and emits a drill receipt artifact.
4. Uses only repo source + the two Actions secrets (vault token, DR key) — by construction zero dependence on the failed host's filesystem.
...
Drill exports: keep 8 weekly. Production nightly: 14 daily, 8 weekly, 12 monthly, plus protected pre-migration/pre-release exports. Deletion is a privileged maintenance action with receipts; models never prune backups. Policy re-derived once real size/change-rate numbers exist (census item still ceremony-gated).
