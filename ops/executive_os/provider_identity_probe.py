...
# Exact PlanType values generated by Codex 0.147.0.  Consumer plans and the
# provider's catch-all are deliberately absent.
...
EXCLUDED_PLAN_TYPES = frozenset(
    {"free", "go", "plus", "pro", "prolite", "unknown"}
)
EXPECTED_AUTH_MODE = {
...
def credential_identity(path: Path) -> dict[str, int]:
    info = path.lstat()
    if (
...
def evaluate_identity(
    *,
    ...
    if auth_mode != EXPECTED_AUTH_MODE[expected_kind]:
        safe["refusal"] = "auth_mode_policy_mismatch"
    elif account_plan in EXCLUDED_PLAN_TYPES or account_plan not in COMPANY_PLAN_TYPES:
        safe["refusal"] = "company_plan_required"
    elif requires is not True:
        safe["refusal"] = "openai_auth_requirement_malformed"
    elif workspace_binding_class != WORKSPACE_BINDING_CLASS:
        safe["refusal"] = "company_workspace_admin_attestation_required"
    else:
        safe["passed"] = True
    return safe
...
def live_probe(
    *, binary: Path, provider_home: Path, expected_kind: str, workspace_binding_class: str
) -> dict[str, Any]:
    before_binary = binary_identity(binary)
    auth_path = provider_home / "auth.json"
    before_credential = credential_identity(auth_path)
    worker_prefix = [
        "/usr/bin/sudo",
        "-n",
        "-u",
    ...
    try:
        ...
        after_credential = credential_identity(auth_path)
