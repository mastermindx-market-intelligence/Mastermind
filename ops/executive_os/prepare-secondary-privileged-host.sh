...
os.chmod(out, 0o400)
PY
install_or_verify_generated_file "$CONFIG_CANDIDATE" "$PRIVILEGED_CONFIG" 0 0 400
...
install_or_verify_generated_file "$ADMIN_CANDIDATE" "$MMX_ADMIN" 0 0 555
...
install_or_verify_generated_file "$POWER_CANDIDATE" "$POWER_LAUNCHER" 0 0 555
...
/usr/bin/plutil -replace Sockets.PrivilegedActions.SockPathMode -integer 432 "$PLIST_CANDIDATE"
...
install_or_verify_generated_file "$PLIST_CANDIDATE" "$PRIVILEGED_PLIST" 0 0 644
...
[ -S "$PRIVILEGED_SOCKET" ]   && [ "$(/usr/bin/stat -f '%u:%g:%Lp' "$PRIVILEGED_SOCKET")" = "$CONTROL_UID:$OPS_GID:660" ]   || refuse "privileged broker socket is absent or has unsafe metadata"
