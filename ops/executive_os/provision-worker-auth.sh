...
if [ "$USE_INSTALLED_CODEX_ATTESTATION" = "true" ]; then
  # The installed binary already crossed the expensive trust boundary once in
  # install.sh. Reuse that sealed receipt and fresh no-follow filesystem
  # identity instead of copying and re-codesigning the ~220 MB executable.
  "$PYTHON_BINARY" -I -S -B - \
    "$SCRIPT_DIR/../.." "$CODEX_ATTESTATION_RECEIPT" \
    "$INSTALLED_CODEX_BINARY" "$WORKER_GID" \
    "$CODEX_VERSION" "$CODEX_TEAM_ID" "$CODEX_SHA256" <<'PY'
import pathlib
import sys
...
worker_gid = int(sys.argv[4])
expected_version, expected_team, expected_sha256 = sys.argv[5:]
...
try:
    attestation = load_codex_attestation_receipt(
        ...
        expected_owner_gid=worker_gid,
    )
except CodexAttestationReceiptError as exc:
    raise SystemExit(f"installed Codex attestation refused: {exc}") from exc

if attestation.version != expected_version:
    raise SystemExit("installed Codex attestation version differs from the allowlist")
if attestation.team_identifier != expected_team:
    raise SystemExit("installed Codex attestation signer differs from the allowlist")
if attestation.sha256 != expected_sha256:
    raise SystemExit("installed Codex attestation digest differs from the allowlist")
PY
  EXECUTABLE_CODEX_BINARY="$INSTALLED_CODEX_BINARY"
else
  # Pre-install enrollment may still originate from a mutable Homebrew/user
  # source. Copy it to a root-owned, non-writable temporary path, then perform
  # the full strict attestation before executing that pinned copy.
