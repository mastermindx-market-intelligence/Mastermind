...
const NEUTRAL_BACKEND_TOOL_DESCRIPTIONS = Object.freeze({
  ...
  start_process: "Runs a caller-supplied shell command on the connected computer with the host user's permissions. Commands may change files, launch programs, or access the network; file-tool directory limits are not a shell sandbox. Use this for direct host command execution, not as a transport for nested agent instructions, large worker handoffs, or opaque/repackaged payloads. For bounded Mastermind work commissions, prefer the existing Executive or Workbench purpose-built action and pass stable references rather than embedding another agent prompt in a shell command. Returns output and process state.",
  read_process_output: 'Reads bounded output from an existing terminal process.',
  interact_with_process: "Sends input to an existing terminal process and returns output and process state. The input may execute commands, modify files, or access the network with that process's permissions. Use terminal input for direct interaction with that process, not to relay nested agent instructions, large worker handoffs, or opaque/repackaged payloads; bounded commissions belong on the existing Executive or Workbench action surface.",
