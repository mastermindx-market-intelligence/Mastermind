...
def build_recovery_indexes(session_truth: Mapping[str,Any], agentos_state: Mapping[str,Any]) -> dict[str,Any]:
    ...
    if isinstance(gh,Mapping) and gh.get("available") is True:
        for pr in gh.get("pull_requests") or []:
            if not isinstance(pr,Mapping): continue
            wid=pr.get("workstream")
            if isinstance(wid,str) and str(pr.get("state") or "").lower() in ACTIVE_PR_STATES:
    ...
    return {"workstreams":workstreams,"waves":waves,"programs":programs,"workstreams_by_program":{k:sorted(v) for k,v in by_program.items()},"session_findings":dict(sf),"program_registry_available":reg.get("available") is True,"github_available":isinstance(gh,Mapping) and gh.get("available") is True,"active_prs":dict(active_prs),"executive_available":executive_available,"executive_active":executive_active}
...
def _typed_gate(row:Mapping[str,Any], *, as_of:str) -> bool:
    if wait_state(row.get("wait") if isinstance(row.get("wait"),Mapping) else None,as_of=as_of)=="VALID": return True
    ...
    for wave in waves:
        ...
        if wait_state(wave.get("wait") if isinstance(wave.get("wait"),Mapping) else None,as_of=as_of)=="VALID": return True
    return False
...
def detect_recovery_findings(session_truth: Mapping[str,Any], agentos_state: Mapping[str,Any], *, as_of: str) -> list[dict[str,Any]]:
    ...
    for ws,row in sorted(idx["workstreams"].items()):
        ...
        waves=row.get("waves") or []
        if waves and all(isinstance(w,Mapping) and str(w.get("status") or "").lower() in TERMINAL_WAVE for w in waves): out.append(_finding("ACTIVE_BUT_COMPLETE",ws,"CEO_ATTENTION",workstream=ws,program=program,action="Repair Agent OS organizational status; do not create a new build."))
        if codes & PROOF_DEBT: out.append(_finding("MERGED_PROOF_DEBT",ws,"CEO_ATTENTION",workstream=ws,program=program,owner="GitHub / proof owner",action="Complete the declared production-proof gate before closing the parent."))
        if "SUPERSEDED_NEXT_ACTION" in codes: out.append(_finding("SUPERSEDED_NEXT_ACTION",ws,"CEO_ATTENTION",workstream=ws,program=program,action="Repair the durable next_action from newer canonical evidence."))
        if codes & UNCLAIMED: out.append(_finding("UNCLAIMED_COMMISSION",ws,"RECOVERY_REQUIRED",workstream=ws,program=program,action="Rebind pre-START capacity or admit a fresh receiver; delivery alone is not START."))
        # Positive frontier evidence suppresses recovery. Quietness/advisory claim absence never creates it.
        if idx["active_prs"].get(ws) or _typed_gate(row,as_of=as_of): continue
        # A fresh scoped Executive observation with zero active operations plus a current GitHub read is
        # positive evidence that no canonical execution/carrier explains this unfinished frontier.
        if idx["github_available"] and idx["executive_available"] and not idx["executive_active"]:
        ...
        elif not idx["executive_available"]:
            if not any(f.get("workstream")==ws and f["disposition"]=="UNKNOWN_RECONCILE" for f in out): out.append(_finding("RUNTIME_OWNERSHIP_UNKNOWN",ws,"UNKNOWN_RECONCILE",workstream=ws,program=program,owner="Executive OS / RuntimeBinding",action="Restore current runtime visibility before deciding whether the frontier is abandoned."))
